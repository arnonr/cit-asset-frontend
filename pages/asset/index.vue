            ...
            <div class="row">
              <div class="col-12">
                <label for="code" class="col-form-label"
                  ><span class="text-danger">*</span>เลือกรูป :</label
                >
                <input
                  ref="add_images_file"
                  class="form-control"
...
const onImportSubmit = async (type) => {
  ...
  const convertImportDate = (date) => {
    ...
    let new_date = date_format[2] + "-" + date_format[1] + "-" + date_format[0];
      ...
      data.push({
        ...
        asset_type_name:
          column_index_import.asset_type_name != undefined
            ? result?.result[i][column_index_import.asset_type_name]?.trim()
            : null,
        asset_type_code:
...
const onGenerateQR = (it, size) => {
  ...
  Swal.fire({
    ...
    inputAttributes: {
      autocapitalize: "off",
      style:
        "display: block;margin-right: auto;margin-left: auto; width: 200px;",
    },
    showCancelButton: true,
